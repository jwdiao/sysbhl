<template>
  <div id="DeviceDetail_dianliuxmz" style="width:100%;height:100%;margin-top:20px;">EchartsLine</div>
</template>
<script>
...
export default {
  ...
  methods: {
    async renderDianliu() {
      let self = this;
      var myChartDidanliu = echarts.init(document.getElementById("DeviceDetail_dianliuxmz"))
      var option = {
        grid: {
          top: 50,
          bottom: 30,
    ...
    async renderDianliu2() {
      ...
      var option = {
        ...
        var data0 = option.series[0].data; // 主轴负载率1 (%)
        var data1 = option.series[1].data; // 主轴负载率2 (%)
        var data2 = option.series[2].data; // 实际主轴转速1 (RPM)
        var data3 = option.series[3].data; // 实际主轴转速2 (RPM)
        ...
        let xAxisData = []
        for(var i=0;i<dataArr.length;i++){
            data0.push(dataArr[i].spinLoad); // 主轴负载率1 (%)
            data1.push(dataArr[i].spinLoad2); // 主轴负载率2 (%)
            data2.push(dataArr[i].actSpin); // 实际主轴转速1 (RPM)
            data3.push(dataArr[i].actSpin2);// 实际主轴转速2 (RPM)
            // data2.push(dataArr[i].actFeed);
            xAxisData.push((dataArr[i].time).substring(11,19))
        }
        option.xAxis[0].data = xAxisData
      this.setIntervalId = setInterval(async () =>{
        ...
        //    console.log('len:',dataArr.length)
          ...
          data0.push(currentSecondData.spinLoad);
          data1.push(currentSecondData.spinLoad2);
          data2.push(currentSecondData.actSpin);
          data3.push(currentSecondData.actSpin2);
