  ...
  <div class="dianliu">
    ...
    <div class="dianliu_main">
        <div class="dianliu_top">
          <div id="deviceDetailBottomEchartsADLJ" style="height:100%;width:100%"></div>
        </div>
        ...
        <div class="dianliu_bottom">
          ...
          <div class="con-NoEmpty" v-else>
            ...
            <div class="main">
              <div id="deviceDetailBottomEchartsBDLJ" style="height:100%;width:100%"></div>
            </div>
          </div>
        </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    // 顶部电流
    this.EchartADom = echarts.init(document.getElementById("deviceDetailBottomEchartsADLJ"))
  ...
  methods: {
    ...
    async getTwoMinutesFun() {
      ...
      if (dataArrDevice.length>0) {
        ...
        this.EchartADom.setOption({
          series: [
            ...
            // xAxis: {
            //     min: function(value) {
            //         // console.log('===444min', value, 'currentTab', currentTab)
            //         if (currentTab === '02'){
            //             return moment().startOf('day').add(8,'hour').format('x');
            //         }
            //         return value.min
            //     },
            //     max: function(value) {
            //         // console.log('===444max', value, 'currentTab', currentTab)
            //         if (currentTab === '02'){
            //             return moment().startOf('day').add(1,'day').add(8,'hour').format('x');
            //         }
            //         return value.max
            //     },
            // }
      ...
      if (this.isElec !== '0') {
          // 底部电流
          if (document.getElementById("deviceDetailBottomEchartsBDLJ")) {
            this.EchartBDom = echarts.init(document.getElementById("deviceDetailBottomEchartsBDLJ"))
            this.AllEchartsArr.push(this.EchartBDom)
            this.renderDianLiuBottom()
            if (dataArrElec.length>0) {
    ...
    async getOneSecondFun() {
        ...
        if(responseOneSecond && responseOneSecond.code === 200 && responseOneSecond.data){
          loading = false
            ...
            if (responseOneSecond.data.elecResponseDtoList.length) {
        ...
        this.EchartADom.setOption({
          series: [{
            ...
            // xAxis: {
            //     min: function(value) {
            //         // console.log('===333min', value, 'currentTab', currentTab)
            //         if (currentTab === '02'){
            //             return moment().startOf('day').add(8,'hour').format('x');
            //         }
            //         return value.min
            //     },
            //     max: function(value) {
            //         // console.log('===333max', value, 'currentTab', currentTab)
            //         if (currentTab === '02'){
            //             return moment().startOf('day').add(1,'day').add(8,'hour').format('x');
            //         }
            //         return value.max
        ...
        if (this.isElec!=='0' && responseOneSecond.data.elecResponseDtoList.length) {
          if (this.EchartBDom) {
            // 电流
            const attr = this.getElecValueByOpt()
    ...
    async renderDianLiuTop() {
      ...
      this.EchartADom.setOption(option, false, false);
    },
    async renderDianLiuBottom() {
      ...
      this.EchartBDom.setOption(option, false, false);
